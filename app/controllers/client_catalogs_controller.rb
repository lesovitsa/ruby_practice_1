class ClientCatalogsController < ApplicationController
    before_action -> { compare_user(catalog_params[:client_id]) }

    def get_all
        @all_products = ClientProduct.all.where(client_id: catalog_params[:client_id])
    ...
    def get_by_product_id
        ...
        @all_products = ClientProduct.all.where(client_id: catalog_params[:client_id]).map { |prod| format_product(prod) }
    ...
    def catalog_params
        params.permit(:client_id, :brand_id, :product_id)
    end
